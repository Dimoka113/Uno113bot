...
def modes(update: Update, context: CallbackContext):
    ...
    modes_explanation = _(
"""
<b>У этого бота UNO есть несколько режимов игры</b>:
...
<b>Так-же в этом боте есть несколько типов игры</b>:
"<b>🤍Белые карты</b>" — в этом типе игры используются обычные, белым картами.
"<b>🖤Чёрные карты</b>" — в этом типе игры используются используются нестандартные, чёрные карты.
"<b>🔀 Flip карты</b>" — в этом типе игры используются сразу 2 колоды карт!
<i>Для того, чтобы поменять колоду, нужно кинуть специальную карту, тогда произойдёт "переворот" и колоды поменяются местами.</i>


<b><i>Учитите, если вы наберёте больше 45 карт вы проиграете!</i></b>
Чтобы изменить режим игры, <u>создателю игры</u> необходимо ввести никнейм бота и пробел,
точно так же, как при розыгрыше карты, и должны появиться все параметры игрового режима.
(<i>Или вы можете воспользоваться кнопками, которые появляются при создании новой игры</i>)
